...
Notation "p => b 'return' 'M' T" := (base (B:=fun _ =>T) p%core (fun _=>b%core) UniMuni) 
  (no associativity, at level 201) : mtac_patt_scope. 
...
Notation "'mtry' a 'with' | p1 | .. | pn 'end'" := 
  (ttry a (fun e=>
    (tmatch (fun _=>mtype_of a) e (cons p1%mtac_patt (.. (cons pn%mtac_patt (cons (base e (fun _ =>raise e) UniRed) nil)) ..)))))
    (at level 82, p1 at level 210, pn at level 210, only parsing).

Notation "'mtry' a 'with' p1 | .. | pn 'end'" := 
  (ttry a (fun e=>
    let ty := fun _ => mtype_of a in
    (tmatch ty e (@cons (tpatt _ ty _) p1%mtac_patt (.. (@cons (tpatt _ ty _) pn%mtac_patt (cons (base e (fun _ =>raise e) UniRed) nil)) ..)))))
    (at level 82, p1 at level 210, pn at level 210, only parsing).

Notation "'mtry' a 'as' e 'in' | p1 | .. | pn 'end'" := 
  (ttry a (fun e=>tmatch (fun _=>mtype_of a) e (cons p1%mtac_patt (.. (cons pn%mtac_patt (cons (base e (fun _=>raise e) UniRed) nil)) ..))))
