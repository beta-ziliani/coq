...
module ArrayRefs = struct
  ...
  let check_close a =
    if Vars.closed0 a then ()
    else Errors.error "Not closed"
    ...
    if Vars.noccurn rel p then
      if noccurn_env env rel then
        try
          ...
          if eq_proof then
            let ex_a = mkProd (Anonymous, a, mkApp(Vars.lift 1 p, [|mkRel 1|])) in
            let px_type = mkApp(p, [|x|]) in
            let ex_p = mkLambda (Anonymous, ex_a, CoqEq.mkAppEq px_type (mkApp(mkRel 1, [|Vars.lift 1 x|])) (Vars.lift 1 y)) in
            let ex_x = t in
