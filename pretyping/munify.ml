...
let is_variable_subs s = 
  Array.fold_right (fun c b -> b && (isVar c || isRel c)) s true
	...
	begin
	  let f (j : int) c  = 
		...
		if (not inside_evar) && (isVar c || isRel c) then
		  begin
		    (if not (Evar.Map.mem ev !map) then
			map := Evar.Map.add ev [j] !map
		     else
			map := Evar.Map.add ev (j :: Evar.Map.find ev !map) !map)
                    ; c
		  end
		else
		  raise Exit
...
and instantiate' ts conv_t env sigma0 (ev, subs as uv) args (h, args' as t) =
  ...
  let res = 
    ...
    let t'' = Evd.instantiate_evar_array evi t' subs in
...
let unify_evar_conv ts env sigma0 cbp t t' =
  match unify ~conv_t:cbp ts env sigma0 t t' with
    | (true, evd) -> Evarsolve.Success evd
    | (false, evd) -> Evarsolve.UnifFailure (evd, Pretype_errors.NotSameHead)
